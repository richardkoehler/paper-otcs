...
void main() {
    ...
    for (image in imageList) {
        if (GROUP = 'Training' && (image.getMetadataValue(key = 'Group') = 'TrainingAnnotated')) {
            detect(image = image, group = GROUP)
            // Now export if wished
            if (EXPORT) {
                export(image = image, group = GROUP, outputFiletype = OUTPUT_FILETYPE)
            }
        } else if (GROUP = 'Results' && (image.getMetadataValue(key = 'Group') = 'Results')) { // && (image.getImageName() = "V19-18;#287;Ebene D - 2021-01-29 13.32.43.ndpi" || image.getImageName() = "V19-18;#336;Ebene E - 2021-01-29 14.29.20.ndpi")) {
...
void detect(image, String group) {
...
def export(image, String group, String outputFiletype) {
